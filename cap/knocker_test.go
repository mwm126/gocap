package cap

import (
	"encoding/hex"
	"github.com/stretchr/testify/assert"
	"testing"
)

type FakeYubikey struct{}

func (yk *FakeYubikey) findSerial() int32 {
	return 5417533
}

func (yk *FakeYubikey) challengeResponse(chal [16]byte) [16]byte {
	var resp [16]byte
	r, _ := hex.DecodeString("2ee619bc248bcefbe09e733d2cdda3be")
	copy(resp[:], r)
	return resp
}

func (yk *FakeYubikey) challengeResponseHMAC(chal SHADigest) [16]byte {
	var hmac [16]byte
	h, _ := hex.DecodeString("2ee619bc248bcefbe09e733d2cdda3be")
	copy(hmac[:], h)
	return hmac
}

func TestPacketFactory(t *testing.T) {
	entropyBuf, _ := hex.DecodeString("280c6d2ea06d20db231dc93bf16db0b7308016d782c7dfe7c969c08cf68cc984")

	var entropy [32]byte
	copy(entropy[:], entropyBuf)

	pk := &PortKnocker{&FakeYubikey{}, [32]byte(entropy)}
	timestamp := int32(1627324072)
	pkt := pk.makePacket("mmeredith", "xUZv!jA&TgHTkw#!3$bUVcDXxW3sY", timestamp)
	hexstring := hex.EncodeToString(pkt)

	assert.Equal(t, len(""), len(hexstring))
	assert.Equal(t, "", hexstring)
}

// func TestKnocker(t *testing.T) {
// 	pk := &PortKnocker{}
// 	pkt := pk.packet()

// 	assert.Equal(t, "TODO", pkt)
// }
